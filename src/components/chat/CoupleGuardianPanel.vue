      ...
      <template v-else>
        <main class="our-space__content">
          ...
          <section v-else class="our-space__phone">
            ...
            <section v-if="snapshot.device.appUsage.length" class="our-space__paper-list our-space__paper-list--blue">
              <header><small>APP TRACE</small><h4>今天打开过什么</h4></header>
              <article v-for="(usage, index) in snapshot.device.appUsage" :key="`${usage.app}-${index}`"><small>{{ usage.lastUsedAt }} · 使用 {{ usage.minutes }} 分钟</small><strong>{{ usage.app }}</strong><p>{{ usage.detail }}</p></article>
            </section>
            <section v-if="snapshot.device.notifications.length" class="our-space__paper-list our-space__paper-list--peach">
              <header><small>NOTIFICATIONS</small><h4>通知栏留下的东西</h4></header>
              <article v-for="(notice, index) in snapshot.device.notifications" :key="`${notice.app}-${notice.time}-${index}`"><small>{{ notice.app }} · {{ notice.time }}{{ notice.unread ? ' · 未读' : '' }}</small><strong>{{ notice.title }}</strong><p>{{ notice.preview }}</p></article>
            </section>
            <section v-if="snapshot.device.chats.length" class="our-space__paper-list our-space__paper-list--lavender">
              <header><small>OUTSIDE CONVERSATIONS</small><h4>聊天之外的联系人</h4></header>
              <article v-for="(chat, index) in snapshot.device.chats" :key="`${chat.contact}-${index}`"><small>{{ chat.relation }} · {{ chat.updatedAt }} · {{ chat.unread ? `${chat.unread} 条未读` : '已读' }}</small><strong>{{ chat.avatarEmoji }} {{ chat.contact }}</strong><p>{{ chat.summary }}</p><div v-if="chat.messages.length" class="our-space__mini-chat"><span v-for="(message, messageIndex) in chat.messages" :key="`${message.time}-${messageIndex}`"><b>{{ message.sender === 'character' ? 'TA' : chat.contact }} · {{ message.time }}</b>{{ message.text }}</span></div></article>
            </section>
            <section v-if="snapshot.device.footprints.length" class="our-space__paper-list our-space__paper-list--green">
              <header><small>SEARCH &amp; BROWSER TRACE</small><h4>搜索、浏览和地图</h4></header>
              <article v-for="(footprint, index) in snapshot.device.footprints" :key="`${footprint.time}-${index}`"><small>{{ footprint.kind }} · {{ footprint.time }}</small><strong>{{ footprint.title }}</strong><p>{{ footprint.detail }}{{ footprint.reason ? `（点开的原因：${footprint.reason}）` : '' }}</p></article>
            </section>
            <section v-if="snapshot.device.gallery.length" class="our-space__paper-list our-space__paper-list--pink">
              <header><small>GALLERY CUTS</small><h4>相册里的生活切片</h4></header>
              <article v-for="(photo, index) in snapshot.device.gallery" :key="`${photo.time}-${index}`"><small>{{ photo.time }}</small><strong>{{ photo.emoji }} {{ photo.title }}</strong><p>{{ photo.detail }}</p></article>
            </section>
            <p v-if="!snapshot.device.notes.length && !snapshot.device.lifeRecords.length && !snapshot.device.appUsage.length && !snapshot.device.notifications.length && !snapshot.device.chats.length && !snapshot.device.footprints.length && !snapshot.device.gallery.length" class="our-space__blank">当前记录没有可展示的手机内容。</p>
          </section>
        </main>
...
.our-space__phone-screen { display: grid; gap: 12px; padding: 15px; border-radius: 21px 21px 7px 21px; color: #fff; background: linear-gradient(145deg, #9895b9, #d6a4af); box-shadow: 0 11px 21px rgba(120,96,119,.13); }.our-space__phone-screen header { display: grid; grid-template-columns: 31px minmax(0, 1fr) auto; align-items: center; gap: 8px; }.our-space__phone-screen header > span { display: grid; width: 31px; height: 31px; place-items: center; border-radius: 10px 10px 4px 10px; background: rgba(255,255,255,.22); }.our-space__phone-screen header div { display: grid; gap: 2px; min-width: 0; }.our-space__phone-screen header small { color: rgba(255,255,255,.76); font-size: 7px; font-weight: 850; letter-spacing: .14em; }.our-space__phone-screen header strong { overflow-wrap: anywhere; font-size: 13px; }.our-space__phone-screen header b { font-size: 17px; }.our-space__phone-network { display: flex; align-items: center; gap: 5px; padding-top: 9px; border-top: 1px solid rgba(255,255,255,.24); font-size: 9px; }.our-space__phone-network span { overflow-wrap: anywhere; }.our-space__phone-network i { margin-left: auto; font-style: normal; }.our-space__paper-list { display: grid; gap: 8px; margin-top: 15px; padding: 14px; border-radius: 16px 16px 7px 16px; background: #fffdf9; box-shadow: inset 0 0 0 1px rgba(230,218,211,.75); }.our-space__paper-list--lavender { background: #fbfaff; box-shadow: inset 0 0 0 1px rgba(223,220,236,.8); }.our-space__paper-list--blue { background: #f7fbff; box-shadow: inset 0 0 0 1px rgba(211,226,242,.8); }.our-space__paper-list--peach { background: #fff9f3; box-shadow: inset 0 0 0 1px rgba(241,222,204,.8); }.our-space__paper-list--green { background: #f7fcf9; box-shadow: inset 0 0 0 1px rgba(210,233,222,.8); }.our-space__paper-list--pink { background: #fff8fb; box-shadow: inset 0 0 0 1px rgba(240,216,226,.8); }.our-space__paper-list article { display: grid; gap: 2px; padding-top: 9px; border-top: 1px dashed #e9dfdc; }.our-space__paper-list article > small { color: #ab999f; font-size: 8px; }.our-space__paper-list article > strong { color: #5d4e56; font-size: 11px; overflow-wrap: anywhere; }.our-space__paper-list article > p { margin: 0; color: #7d6e76; font-size: 9px; line-height: 1.6; white-space: pre-wrap; overflow-wrap: anywhere; }.our-space__mini-chat { display: grid; gap: 6px; margin-top: 7px; padding: 8px; border-radius: 10px; background: rgba(255,255,255,.72); }.our-space__mini-chat span { display: grid; gap: 2px; color: #6e5e66; font-size: 9px; line-height: 1.55; white-space: pre-wrap; overflow-wrap: anywhere; }.our-space__mini-chat b { color: #aa8794; font-size: 8px; font-weight: 700; }
